...
from django.shortcuts import render, HttpResponse, redirect, get_object_or_404, render
...
def create_supplier(request):
    ...
    # Render the create_supplier.html template for GET requests
    return render(request, 'posApp/create_supplier.html')
...
def supplier_list(request):
    ...
    return render(request, 'posApp/supplier_list.html', context)
...
def delete_supplier(request):
    ...
    id = request.POST.get('id')
    try:
        delete = Supplier.objects.filter(id = id).delete()
        resp['status'] = 'success'
        messages.success(request, 'Sale Record has been deleted.')
    except:
        resp['msg'] = "An error occured"
